...
def run_experiments(
    ...
    # 실험 4: 원본 + 전통적 100개 + SD 100개 (하이브리드)
    print("\n" + "="*70)
    ...
    # 전통적 증강에서 클래스당 개 (각 클래스 200개 중 개)
    trad_indices = []
    ...
    # 실험 5: 원본 + 전통적 200개 + SD 200개 (하이브리드)
    print("\n" + "="*70)
    print("Experiment 5: Original + Traditional(200) + SD(200)")
    print("="*70)
    
    # 전통적 증강에서 클래스당 개 (각 클래스 200개 중 200개)
    trad_indices = []
    for class_id in range(10):
        class_count = 0
        for idx in range(len(original_dataset), len(dataset2)):
    ...
    # 합치기
    all_indices = orig_indices + trad_indices + sd_indices
    dataset5 = Subset(dataset2 if len(trad_indices) > 0 else dataset3, all_indices)
    
    history5 = train_and_evaluate(
        dataset5, test_dataset, model_fn,
        epochs=epochs, batch_size=batch_size, lr=lr,
        save_path=output_path / 'exp5_best.pth'
    )
    
    results['exp5_hybrid'] = {
        'train_size': len(dataset5),
        'best_acc': history5['best_acc'],
        'final_acc': history5['test_acc'][-1]
    }
